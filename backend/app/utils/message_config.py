...
def get_smtp_settings(organization_id):
    ...
    if not organization:
        app_logger.critical(f"get_smtp_settings: Organization ID [{organization_id}] not found.")
...
def send_email(subject, recipients, body, organization_id):
    try:
        ...
        msg.body = body
        mail.send(msg)
        app_logger.info(f"send_email - Message {subject} to {recipients} has been sent.")
        return True  # Email sent successfully
    except smtplib.SMTPException as e:
        # Handle specific SMTP errors
        app_logger.critical(f"send_email - SMTP error: {e}")
        return False
    except Exception as e:
        # Handle any other exception
        app_logger.critical(f"send_email - Failed to send email: {e}")
        return False
